...
description: "Creating a function that shows you which Exclusive Economic Zones along the U.S. West Coast have the most potential for developing marine aquaculture"
author:
...
To create a map the `potential_aquaculture_area` data was added to `economic_zones` data. A map of EEZ regions colored by amount of suitable area for farming oysters was then created. A table was then made using the `kable()` function to display the potential marine aquaculture area(km²) for each EEZ.
...
[SeaLifeBase](https://www.sealifebase.ca/search.php) is a good website to use to find information on species depth and temperature requirements.
...
```{r}
# Use suitable_eez() function on Red abalone
...
[@noaa_coralreefwatch_2018] [@gebco_2025_grid_2025] [@flanders_marine_eez_2025] [@sealifebase_2025] [@gsa_aquaculture_2019]
